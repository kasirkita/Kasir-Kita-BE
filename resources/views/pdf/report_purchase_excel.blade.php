...
<body>
    <table>
        <thead>
            <tr>
                <th colspan="6" style="text-align: center">Laporan Pembelian Barang dari tanggal {{ Carbon\Carbon::parse($start_date)->format('d/m/Y') }} s/d {{ Carbon\Carbon::parse($end_date)->format('d/m/Y') }} </th>
            </tr>
        </thead>
        <thead>
            <tr>
        ...
        <tbody>
            @foreach ($purchases as $purchase)
                <tr>
                    <td>{{ $purchase->purchase->payment_date }}</td>
                    <td>{{ $purchase->product_name }}</td>
                    <td style="text-align: right">{{ $purchase->price }}</td>
                    <td style="text-align: right">{{ $purchase->cost }}</td>
                    <td>{{ $purchase->qty }}</td>
        ...
        <tfoot>
            <tr>
                ...
                <th style="text-align: right">{{ $purchases->sum('qty') }}
            </tr>
            <tr>
                <th colspan="5" style="text-align: right">Total Pembelian</th>
                <th style="text-align: right">{{ $purchases->sum('subtotal') }}
            </tr>
        </tfoot>
    </table>
</body>
